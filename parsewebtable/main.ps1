...
class HtmlElement {
    ...
    [void] MethodName([string]$htmltext) {
        
    }
    [HtmlElement] GetFirstElementbyName([string]$tagname) {
        $ignorespace = " "
        $tagname = "div"
        $endtag = ""
        $tagnameindex = 0
        ...
        $foundCountent = ""
        $endtag = "/$tagname>"
        foreach ($item in $_content -split "") {
            $foundCountent += $item
            if ($item -ne $ignorespace) {
                if ($isFoundPrefix) {
                    if ($isReach) {
                        if ($endtag[$endtagindex] -eq $item) {
                            if ($endtag.Length -eq ($endtagindex + 1)) {
                                $isFound = $true
                                $isReach = $false
                                break
                            }
                    ...
                    if ($isReach) {
                        if ($tagname[$tagnameindex] -eq $item) {
                            if ($tagname.Length -eq ($tagnameindex + 1)) {
                                $isFoundPrefix = $true
                                $isReach = $false
                            }
                            else {
...
[System.Management.Automation.PSObject]$response = Invoke-WebRequest https://www.videocardbenchmark.net/common_gpus.html
# [System.Management.Automation.PSObject]$response = Invoke-WebRequest https://www.google.com

$content = $response.Content


# $tag = $content.ParsedHtml.IHTMLDocument3_getElementsByTagName("div")
exit
